<template>
    <div>
        <ul class="list-group center">
            ...
            <li v-for="(note, index) in notes" class="list-group-item" v-if="notesLength" :key="index">
                <div class="row">
                    <div class="col-sm-7">
                        <span v-if="currentEditingId !== note.id">{{ note.title }}</span>
                        <form v-if="currentEditingId === note.id && editing" @submit.prevent="update">
                            <input type="text" v-model="note.title" :v-model="editedTitle">
                            <button type="submit">Update</button>
                        </form>
                    </div>
                    &nbsp;
                    <div class="options col-sm-4 pull-right text-right">
                        <div class="glyphicon glyphicon-trash" @click="deleteNote(note.id)" title="Delete Note"></div>
                        <div class="glyphicon glyphicon-pencil" @click="edit(note.id)" v-if="currentEditingId !== note.id" title="Edit Note"></div>
                        <div :class="favouriteClass(note.is_favourite)" @click="toggleFavourite(note.id, note.is_favourite)" :title="favouriteTitle(note.is_favourite)"></div>
                    </div>
                </div>
            </li>
